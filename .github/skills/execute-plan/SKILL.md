...
- 本スキルは「依存グラフ解析と並列実行の構造化手順」を提供する。CLI ビルトインの **Autopilot mode**（Shift+Tab）は tool 承認をスキップする独立機能で、両者は併用可能
- 単発の小タスクなら Autopilot のみで十分だが、複数タスク・依存関係・Board 統合がある場合は本スキルを使う
