...
| 計画策定 | manager | `manager` | analyst + impact-analyst の結果を統合して計画 |
...
```
SEQUENTIAL:
  - architect エージェント呼び出し（条件付き — escalation 推奨時）
  - manager エージェント呼び出し（analyst + impact-analyst + architect の結果を入力）
```
...
- `manager` エージェントに実行計画の策定を依頼する（analyst + impact-analyst + architect の結果を入力に含む）
...
- test-verifier の verdict が `conditional_pass` → manager に許容判断を委ねる
