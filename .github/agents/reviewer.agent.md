...
レビュー時に以下を少なくとも1つ検討する:

- **逆効果シナリオ**: この変更が意図しない悪影響を及ぼす条件はないか？
- **目的不適合**: 技術的に正しいが、依頼の本来目的を満たしていないケースはないか？
- **過剰実装**: 依頼されていない機能を追加していないか？
- **前提崩壊**: 暗黙の仮定が成り立たない場合に何が壊れるか？

> **Why**: Verification（テスト pass）だけでは Validation（目的適合）を保証できない。
> レビュアーとして「正しいものを作っているか」の視点を提供する。
