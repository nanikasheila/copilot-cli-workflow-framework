...
- **受け入れ基準（AC）**: 各要求の完了条件（テスト可能な形式・`requirements_development.success_metrics` への参照を含む）
- **エッジケース（EC）**: 境界値・異常系・競合条件
- **時間的振る舞い**: 複数サイクル・収束/過渡応答・統合的振る舞いの期待
...
4. **受け入れ基準の策定**: 各 FR に対してテスト可能な AC を定義
   - 可能な限り `requirements_development.validated_needs.success_metrics` の SM-xxx を AC 文中で参照する（例: `AC-001: ログイン応答が SM-001 を満たすこと（< 300ms）`）
   - 曖昧表現（「速い」「適切」）は禁止。観察可能な形に変換する
5. **時間的振る舞いの抽出**: 複数サイクル動作・収束/過渡応答・統合的振る舞いがある場合は、対応する AC または EC として明示する（rules/verification-procedures.md §3a 参照）
6. **スコープの明確化**: 対象範囲と対象外を明示
